...
(function (root) {
    ...
    function create(profile, fields, clock = Date.now()) {
        ...
        const transactions = amounts.map((valor,i)=>({
            ...values, id:start+i, serieId:recurrence === 'nenhuma' ? null : start,
        ...
        return {transactions,contaId:values.contaId,cartaoId:values.cartaoId,cardIncrease:values.tipo === 'despesa-cartao' ? transactions.reduce((sum,t)=>sum+finance().cents(t.valor),0)/100 : 0};
    }
